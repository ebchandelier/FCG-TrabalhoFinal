...
GLuint BuildTriangles(); // Constrói um triângulo para renderização
...
int main()
{
    ...
    window = glfwCreateWindow(800, 800, "Player 1", NULL, NULL);
    ...
    window2 = glfwCreateWindow(800, 800, "Player 2", NULL, window);

    glfwSetKeyCallback(window2, KeyCallback);
    glfwSetMouseButtonCallback(window2, MouseButtonCallback);
    glfwSetCursorPosCallback(window2, CursorPosCallback);
    glfwSetScrollCallback(window2, ScrollCallback);
    glfwSetFramebufferSizeCallback(window2, FramebufferSizeCallback);
    glfwSetWindowSize(window2, 800, 800); // Forçamos a chamada do callback acima, para definir g_ScreenRatio.
   ...
   glBindVertexArray(vertex_array_object_id);
    ...
    while (!glfwWindowShouldClose(window) || !glfwWindowShouldClose(window2))
    {
        ...
        // Veja slide 194 do documento "Aula_08_Sistemas_de_Coordenadas.pdf".
        // 
        ...
        //CABEÇA
        PushMatrix(model);
            model = model * Matrix_Translate(0.0f, 0.1f, 0.0f);
            ...
            PushMatrix(model); 
	        	...
	        	PushMatrix(model); 
                    ...
                    PushMatrix(model);
                        model = model * Matrix_Translate(0.0f, -0.65f, 0.0f);
                        PushMatrix(model);
                            model = model * Matrix_Scale(0.2f, 0.05f, 0.2f);
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            ...
            PushMatrix(model); 
	        	...
	        	PushMatrix(model); 
                    ...
                    PushMatrix(model);
                        model = model * Matrix_Scale(0.2f, 0.6f, 0.2f); 
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); 
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO ESQUERD
                    PopMatrix(model); 
        ...
        PushMatrix(model);
            ...
            //CANELA DIERITA
            PushMatrix(model);
                model = model * Matrix_Translate(0.0f, -0.9f, 0.0f);
                PushMatrix(model);
                     model = model * Matrix_Scale(0.3f, 0.6f, 0.3f);
                     //model = model * Matrix_Translate(0.0f, -0.1f, 0.0f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform); //CANELA DIREIAT
                PopMatrix(model);

                //Pé DIREITO
                PushMatrix(model);
                    model = model * Matrix_Translate(0.0f, -0.7f, 0.1f);
                    model = model * Matrix_Scale(0.3f, 0.2f, 0.5f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        PushMatrix(model);
            ...
            PushMatrix(model);
                ...
                PushMatrix(model);
                    ...
                    model = model * Matrix_Scale(0.3f, 0.2f, 0.5f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform);//Pé ESQUERDA
                    PopMatrix(model);
                PopMatrix(model);
            PopMatrix(model);
        PopMatrix(model);
        ...
        // Veja slide 133 do documento "Aula_08_Sistemas_de_Coordenadas.pdf".
        ...
        TextRendering_ShowEulerAngles(window2);
        // Imprimimos na informação sobre a matriz de projeção sendo utilizada.

        TextRendering_ShowProjection(window);
        TextRendering_ShowProjection(window2);
        // Imprimimos na tela informação sobre o número de quadros renderizados
        // por segundo (frames per second).

        TextRendering_ShowFramesPerSecond(window);
        TextRendering_ShowFramesPerSecond(window2);
...
GLuint BuildTriangles()
{
    ...
    //  - slide 133 do documento "Aula_08_Sistemas_de_Coordenadas.pdf";
    //  - slide 135 do documento "Aula_09_Projecoes.pdf".
    //
    // Este vetor "model_coefficients" define a GEOMETRIA (veja 134 do
    ...
    // de textura.  Cada tipo de atributo precisa de um VBO independente.
    // Abaixo criamos um VBO para armazenarmos um atributo: posição.
    ...
    // cada vértice: uma cor (veja slide 135 do documento "Aula_04_Modelagem_Geometrica_3D.pdf").
    ...
    // model_coefficients.
    //
    // Para referência sobre os modos de renderização, veja slide 175 do
    // documento "Aula_04_Modelagem_Geometrica_3D.pdf".
    //
    // Este vetor "indices" define a TOPOLOGIA (veja slide 134 do documento
    ...
    // Finalmente, copiamos os valores do array indices[] para dentro do buffer.
    ...
    // Retornamos o ID do VAO. Isto é tudo que será necessário para renderizar
